...
public class Transform {
	...
	/*public static Transform parentTransform(Transform transform, Transform transform2) {
		return new Transform(Vector2.add(transform.position, transform2.position),
				transform.rotation + transform2.rotation, Vector2.multiply(transform.scale, transform2.scale));
	}

	public static Transform toWindowCords(Transform transform) {
		Transform trans = new Transform(Vector2.add(Vector2.substract(transform.position, Camera.activeCam.gameObject.getTransformWithCaution().position), Vector2.divide(GameContainer.windowSize, 2)), transform.rotation, transform.scale);
		return trans;
	}*/
