...
public class TransformController extends UIGameBehaviour implements ButtonListener {
	...
	public void ButtonClicked() {
		if (this.state == 2 || this.state == 3 || this.state == 4) { // no "if" if its acitivaded cause this would ask
																		// ethen more
			this.state = 0;
			...
			state = 0;
			if (!this.features[0] || ((this.features[1] || this.features[2])
					&& Math.abs(Vector2.substract(newMousePos, this.oldMousePos).length()) <= 2)) { // Not moving
																									// mouseMuch
	...
	public void ButtonPress() {
		if (this.state == 2 || !this.features[0]) { // If the button is clicked you dont want to move it || position
													// feature is deactivaded
			return;
		}
	...
	public void update() {
		...
		if (this.state == 1) {
			this.gameObject
					.setPosition(Vector2.substract(mousePos, Vector2.substract(this.oldMousePos, this.oldObjectPos)));
			...
			if (this.features[1] && GameContainer.input.isKeyDown(KeyEvent.VK_R)) {
				...
				this.oldMouseObjectDistance = Vector2
						.substract(this.gameObject.getTransformWithCaution().position, mousePos).length();
			}
			...
			float mouseObjectDistance = Vector2.substract(this.gameObject.getTransformWithCaution().position, mousePos)
					.length();
