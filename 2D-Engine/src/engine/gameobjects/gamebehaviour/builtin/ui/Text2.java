...
public class Text2 extends UIGameBehaviour {
	...
	private Vector2 alignment = Text2.CENTER;
	...
	public Text2(String text, int size, Color c) {
		this.text = text;
	...
	private void textUpdate() {
		...
		//Y-Alignmnet
		if (this.alignment.y == Text2.CENTER_int) { //CENTER
			this.offset.y = (-this.bounds.getSize().y + this.realSize) / 2;
		} else if (this.alignment.y == Text2.LEFT_int) { //TOP
			this.offset.y = this.bounds.getPoint1().y + this.realSize;
		} else if (this.alignment.y == Text2.RIGHT_int) { //BOTTON
			this.offset.y = this.bounds.getPoint2().y + this.realSize;
		}
		
		System.out.println(this.bounds.getPoint1() + " " + this.bounds.getPoint2());
		
		//X-Alignment
		width = this.d.getFontSize(this.font).stringWidth(text) * this.realSize;
		if (this.alignment.x == Text2.CENTER_int) { //CENTER
			this.offset.x = -this.bounds.getSize().x/2 - width / 2;
		} else if (this.alignment.x == Text2.LEFT_int) { //LEFT
			this.offset.x = -this.bounds.getSize().x;
		} else if (this.alignment.x == Text2.RIGHT_int) { //Right
			this.offset.x = -width;
		}
