...
public abstract class GameBehaviour implements Serializable {
	...
	public boolean testInWorldStateSame() { //Test if the state of the Behaviour is compatible with the from gameObject
		if (prefferedInWorldState == 2) {
			return true;
		} else if ((this.gameObject.getInWorld() ? 1 : 0) != prefferedInWorldState) { //Behaviour not compatible ! Sorry
			this.gameObject.removeComponent(this);
			System.err.println(this.getClass() + " no compatible with GameObjects inWorld State:" + this.gameObject.getInWorld());
			return false;
		}
		return true;
	}
