...
public abstract class Scene {
	...
	private boolean inView(GameObject obj) {
		if (obj == Camera.activeCam.gameObject || !obj.inWorld) {
			...
			if (!(obj.getTransformWithCaution().position.x + viewRangeX <= Camera.activeCam.gameObject.getTransformWithCaution().position.x - windowBounds.x ||
				   obj.getTransformWithCaution().position.x - viewRangeX >= Camera.activeCam.gameObject.getTransformWithCaution().position.x + windowBounds.x ||
				   obj.getTransformWithCaution().position.y + viewRangeY <= Camera.activeCam.gameObject.getTransformWithCaution().position.y - windowBounds.y  ||
			 	   obj.getTransformWithCaution().position.y - viewRangeY >= Camera.activeCam.gameObject.getTransformWithCaution().position.y + windowBounds.y)) {
				return true;
			} else {
				return false;
